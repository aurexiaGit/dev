// Currently error : not sent to all three addresses
...
function initialDonations() {
	...
	if (window.confirm("Do you really want to send initial donations to all Blockchain members?")) {
		for (key in users) {
			Token.mintToken(users[key].address,50*Math.pow(10,-18),function(err,result) {console.log("")}) // The initial donations were fixed at 50 AST
		}
	}
}